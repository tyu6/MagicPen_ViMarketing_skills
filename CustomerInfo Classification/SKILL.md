...
description: Analyze sales, business-development, support, or account-management conversation transcripts and grade customer opportunity as high, medium, or low with structured evidence. Use when the input is a readable dialogue file and the user wants an explainable JSON assessment plus an optional Markdown summary.
version: 1.1.0
user-invocable: true
...
- `.pdf` only supports PDFs with an embedded text layer
...
Optional Markdown summary should contain:
...
4. To generate Markdown too, use one of these patterns
...
5. If the file is `.docx`, `.doc`, or `.pdf`, let the script extract text first. If extraction fails, report the specific limitation.

## Scoring Model

The analyzer uses eight dimensions and a 100-point total score. Full details live in [references/scoring-rubric.md](references/scoring-rubric.md).

| Dimension | Max | Interpretation |
| --- | ---: | --- |
| 需求明确度 | 15 | Need, pain point, and scenario clarity |
| 采购意愿 | 20 | Buying, trial, quotation, contract, or PoC intent |
...
| 落地可行性 | 10 | Deployment, integration, and implementation readiness |
...
- `medium`: total score `35-69`, or strong interest exists but budget, timeline, or decision chain is still incomplete
- `low`: total score `< 35`, or severe blockers appear, such as explicit rejection, no budget, no plan, or obvious stalling

### Strong Signal Rules

- Up-weight explicit budget, explicit timeline, decision-maker appearance, PoC or trial request, quotation or contract request, implementation owner, and technical docking owner
- Down-weight explicit rejection, no budget, no plan this quarter or year, repeated cooling language, free-consulting behavior, or hard compliance and IT blockers
- If the evidence is sparse, keep the judgment conservative and reduce confidence

## Noise Handling

- Strip obvious system noise first
- Prefer lines likely spoken by the customer
...
```json
{
  ...
  "confidence": 80,
  "reasoning": "客户已明确提出 PoC、预算、报价和实施计划，需求和推进路径都较清晰。"
}
```
...
```json
{
  ...
  "confidence": 60,
  "reasoning": "存在明确兴趣和部分场景，但仍处于比较方案阶段，预算和时间尚未明确。"
}
```
...
- Use [scripts/analyze_customer_opportunity.py](scripts/analyze_customer_opportunity.py) for actual scoring and output generation
...
- Use [references/scoring-rubric.md](references/scoring-rubric.md) when you need the detailed meaning of each dimension
...
- Preserve original wording in `evidence` wherever possible, but keep the list short and high signal
